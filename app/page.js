...
export default function TaskFlow() {
  ...
  if (loading || !user) {
    return <div style={{ padding: '50px', textAlign: 'center', fontFamily: 'sans-serif', color: '#64748b' }}>Yükleniyor...</div>;
  }
  ...
  return (
    <div style={{ padding: '40px 0', backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '1000px', margin: '0 auto 50px', padding: '0 20px' }}>
        <h1 style={{ color: '#0f172a', fontWeight: '900', fontSize: '32px', margin: 0, letterSpacing: '-0.02em' }}>
          TaskFlow <span style={{ color: '#2563eb' }}>Kanban</span>
        </h1>
      ...
      <DndContext sensors={sensors} collisionDetection={rectIntersection} onDragStart={(e) => setActiveId(e.active.id)} onDragEnd={handleDragEnd}>
        <div style={{ 
          display: 'flex', 
          gap: '30px', 
          justifyContent: 'safe center', // Kritik düzeltme: İçerik dışarı taşarsa en sola yasla
          alignItems: 'flex-start', 
          overflowX: 'auto', 
          width: '100%',
          paddingBottom: '40px',
          paddingLeft: '20px', // Kenarlardan taşmayı önlemek için
          paddingRight: '20px',
        ...
        <DragOverlay>
          {activeId ? (
            <TaskCard 
              title={Object.values(columns).flatMap(c => c.items).find(i => i.id === activeId)?.content} 
              isOverlay 
            />
          ) : null}
        </DragOverlay>
  ...
  async function handleDragEnd(event) {
    ...
    if (activeCol !== overCol) {
      ...
      setColumns(prev => {
        ...
        return { 
          ...prev, 
          [activeCol]: { ...prev[activeCol], items: arrayMove(items, oldIdx, newIdx) } 
        };
      });
    }
  }
